...
4. If Matomo credentials detected (`"$HOME/.claude/skills/seo/scripts/claude-seo" run matomo_auth.py --check`), also spawn seo-matomo agent (alternative or complement to seo-google's GA4 reports)
...
7. If backlink APIs detected (`"$HOME/.claude/skills/seo/scripts/claude-seo" run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
11. If drift baseline exists for this URL (`"$HOME/.claude/skills/seo/scripts/claude-seo" run drift_history.py <url>`), also spawn seo-drift agent
