...
- **Lighthouse 13.5.0** (September 2026, latest stable): Lighthouse 13.0 (Oct 2025) migrated performance audits to **insight-based audits** aligned with the DevTools Performance panel and removed legacy audits (first-meaningful-paint, font-size, third-party-facades). The performance *score* remains metric-based and was **not** re-weighted. 13.2.0-13.3.0 added and default-enabled a new **Agentic Browsing** category (Chrome 150+; fractional pass-ratio). 13.4.1 enabled it through the PSI API; the CLI needs Node.js 22.19 or newer. Details: `C:/Users/jcuad/.claude/skills/seo-agentic/references/lighthouse-agentic-category.md`. Lighthouse is a lab tool (simulated conditions): always cross-reference with CrUX field data.
